  ...
  Router.get(
    ...
    async (
      ...
      res: {
        ...
        setStatus: (arg0: number) => {
          (): any;
          new (): any;
          json: {
            (arg0: { status: string; timestamp: string; database: string; error: string }): void;
            new (): any;
          };
        };
      }
...
function registerApiV1Routes(
  ...
  Router.group(router, '/api/v1', (r: any) => {
    // Auth routes
    Router.post(
      r,
      '/auth/login',
      async (_req: any, res: { json: (arg0: { message: string }) => void }) => {
    ...
    // Posts resource
    Router.get(r, '/posts', async (_req: any, res: { json: (arg0: { data: never[] }) => void }) => {
      res.json({ data: [] });
    });
    ...
    Router.get(
      ...
      '/posts/:id',
      async (
        req: { getParam: (arg0: string) => any },
        res: { json: (arg0: { data: { id: any } }) => void }
      ) => {
...
function registerAdminRoutes(router: IRouter): void {
  Router.group(router, '/admin', (r: any) => {
    Router.get(
      r,
      '/dashboard',
      async (_req: any, res: { json: (arg0: { message: string }) => void }) => {
    ...
    Router.get(r, '/users', async (_req: any, res: { json: (arg0: { data: never[] }) => void }) => {
      res.json({ data: [] });
    });
  });
}

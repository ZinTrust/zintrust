...
const isWranglerD1Driver = (driver: string): boolean => driver === 'd1';
...
const warnIfAdapterMissing = (
  ...
  if (conn.driver === 'mysql' && DatabaseAdapterRegistry.get('mysql') === undefined) {
    ...
    cmd.debug(
      '[debug] Expected a side-effect import in src/zintrust.plugins.ts like: import "@zintrust/db-mysql/register";'
    );
  }
  if (conn.driver === 'postgresql' && DatabaseAdapterRegistry.get('postgresql') === undefined) {
    cmd.warn(
      'PostgreSQL adapter is not installed/registered; migrations may not hit a real PostgreSQL DB.'
    );
    cmd.warn('Install via `zin plugin install adapter:postgres` (or `zin add db:postgres`).');
  }
};

const withD1RemoteSqlMode = async <T>(fn: () => Promise<T>): Promise<T> => {
  if (typeof process === 'undefined' || process.env === undefined) return fn();

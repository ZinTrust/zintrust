...
export class CloudflareAdapter implements RuntimeAdapter {
  ...
  async handle(event: unknown): Promise<PlatformResponse> {
    try {
      ...
      const { req, res, responseData } = this.createMockHttpObjects(platformRequest);
      ...
      try {
        ...
        await this.config.handler(req as IncomingMessage, res as ServerResponse, body);
      } finally {
      ...
      this.logger?.debug('Cloudflare request processed', {

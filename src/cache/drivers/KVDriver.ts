...
export class KVDriver implements CacheDriver {
  ...
  constructor() {
    ...
    const env = (globalThis as Record<string, unknown>)['env'] as
      ...
      | undefined;
    this.kv = env === undefined ? undefined : (env['CACHE'] as KVNamespace);
  }

  public async get<T>(key: string): Promise<T | null> {
    if (this.kv === undefined) return null;
  ...
  public async has(key: string): Promise<boolean> {
    if (this.kv === undefined) return false;

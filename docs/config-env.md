...
| `TRACE_SERVICE_TAG`                          | `APP_NAME`         | Global tag appended once to outgoing trace entries; falls back to `APP_NAME` when empty.     |
| `TRACE_PROXY`                                | `false`            | Send trace writes to a signed remote trace gateway instead of writing directly to local DB.  |
| `TRACE_PROXY_URL`                            | empty              | Base URL for the remote trace ingest server when `TRACE_PROXY=true`.                         |
| `TRACE_PROXY_PATH`                           | `/zin/trace/write` | Signed trace ingest path on the remote server.                                               |
| `TRACE_PROXY_KEY_ID`                         | `APP_NAME`         | Signing key ID used by trace proxy requests and ingest verification.                         |
| `TRACE_PROXY_SECRET`                         | `APP_KEY`          | Signing secret used by trace proxy requests and ingest verification.                         |
| `TRACE_PROXY_TIMEOUT_MS`                     | `30000`            | Request timeout in milliseconds for outgoing trace proxy writes.                             |
| `TRACE_PROXY_SIGNING_WINDOW_MS`              | `60000`            | Allowed signature timestamp skew for incoming trace ingest verification.                     |
| `TRACE_PROXY_NONCE_TTL_MS`                   | `120000`           | Nonce TTL used for replay protection on the trace ingest server.                             |
| `TRACE_PROXY_MIDDLEWARE`                     | empty              | Optional comma-separated middleware list applied to `registerTraceIngestGateway(...)`.       |
...
| `BULLETPROOF_DEVICE_DB_CONNECTION` | `default`                      | DB connection name used by the core Bulletproof device store.     |
| `BULLETPROOF_DEVICE_DB_TABLE`      | `zintrust_bulletproof_devices` | Table name for core Bulletproof device secrets.                   |

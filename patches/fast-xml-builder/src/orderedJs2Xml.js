...
import { escapeAttribute, escapeAttributeRaw, safeCdata, safeComment } from './util.js';
  ...
  if (attrMap && !options.ignoreAttributes) {
    for (let attr in attrMap) {
      ...
      if (attrVal === true && options.suppressBooleanAttributes) {
        ...
        attrStr += ` ${attr.substr(options.attributeNamePrefix.length)}="${escapeAttributeRaw(attrVal)}"`;
      }
    }
  }
  return attrStr;

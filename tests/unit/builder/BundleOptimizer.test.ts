...
describe('BundleOptimizer', () => {
  ...
  describe('Bundle Analysis', () => {
    it('should analyze bundle with multiple files', async () => {
      ...
      vi.mocked(fs.statSync).mockImplementation((filePath: any) => {
        if (filePath.includes('file1')) return { size: 1000 } as any;
        if (filePath.includes('file2')) return { size: 2000 } as any;
        if (filePath.includes('file3')) return { size: 3000 } as any;
    ...
    it('should calculate file percentages correctly', async () => {
      ...
      vi.mocked(fs.statSync).mockImplementation((filePath: any) => {
        if (filePath.includes('large')) return { size: 8000 } as any;
        if (filePath.includes('small')) return { size: 2000 } as any;
        return { size: 0 } as any;
      });
    ...
    it('should sort files by size descending', async () => {
      ...
      vi.mocked(fs.statSync).mockImplementation((filePath: any) => {
        if (filePath.includes('large')) return { size: 3000 } as any;
        if (filePath.includes('medium')) return { size: 2000 } as any;
        if (filePath.includes('small')) return { size: 1000 } as any;
        return { size: 0 } as any;
      });
  ...
  describe('Recommendations', () => {
    ...
    it('should generate warning for large files', async () => {
      ...
      vi.mocked(fs.statSync).mockImplementation((filePath: any) => {
        if (filePath.includes('large')) return { size: 30 * 1024 * 1024 } as any;
  ...
  describe('File Recursive Traversal', () => {

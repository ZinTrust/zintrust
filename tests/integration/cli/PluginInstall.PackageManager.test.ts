...
describe('CLI → Plugin install with package manager', () => {
  ...
  beforeAll(async () => {
    ...
    await (await import('node:fs/promises')).writeFile(join(tmp, 'package.json'), JSON.stringify({ name: 'p' }), 'utf8');
    process.chdir(tmp);
  });
  ...
  it('runs pnpm when --package-manager pnpm is provided', async () => {
    ...
    const calls = vi.mocked(SpawnUtil.spawnAndWait).mock.calls.map((c) => ({ cmd: c[0].command, args: c[0].args }));

    expect(calls.some((c) => c.cmd === 'pnpm' && c.args.includes('add') && c.args.includes('a') && c.args.includes('b'))).toBe(true);
    expect(calls.some((c) => c.cmd === 'pnpm' && c.args.includes('add') && c.args.includes('d') && c.args.includes('-D'))).toBe(true);
